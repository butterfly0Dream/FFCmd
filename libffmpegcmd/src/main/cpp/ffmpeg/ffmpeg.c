...
int is_cancel = 0;//标记 已经退出

int is_run = 0;//标记 正在执行命令行 如果没有执行 点击取消会出现异常
...
static int transcode(void) {
    ...
    while (!received_sigterm) {
        av_log(NULL, AV_LOG_VERBOSE, "解码主循环");
        //2020 在这里标记暂停
        if (is_cancel)
            goto fail;
...
//带进度回调的 执行命令行
int jxRunPro(int argc, char **argv,void log_callback_cpp(void *ptr, int level, const char *fmt, va_list vl)) {
    ...
    //设置日志
    av_log_set_callback(log_callback_cpp);
